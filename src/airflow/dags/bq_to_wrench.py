...
default_args = {
    'owner': 'airflow',
    'description': 'Offload data from BQ to GCS; Upload GCS files to Wrench /upload endpoint.',  # noqa: E501
...
tables = [
    {
        ...
        'select': [
            'icentris_client', 'first_name', 'last_name', 'email', 'email2', 'email3',
...
def insert_status_record(icentris_client, status, table, file, datetime_now):
    bq_client = BigQuery(env)
    query = build_status_insert(icentris_client, status, table, file, datetime_now)
    bq_client.query(query)


def build_status_insert(icentris_client, status, table, file, start_date):
    return BigQuery.querybuilder(
        table='wrench.data_source_status',
        insert=[
            {'icentris_client': icentris_client, 'file': file, 'table': table,
    ...
    with tempdir() as tmp:
        ...
        for blob in blobs:
            ...
            if _is_cloud_storage_dir(key):
                ...
                icentris_client = blob.name.split('-')[1]
                api_key = WrenchAuthWrapper.instance(env).get_api_key(icentris_client)

                log.info(f'Uploading {key} to Wrench')
                ...
                if (response.status_code != requests.codes.ok):
                    try:
                        ...
                        raise Exception(f'''Error pushing {key} to Wrench.
                ...
                if (response.json().get('file', None) is None):
                    try:
                        ...
                        raise Exception('Wrench response did not contain the "file" attribute')
                ...
                insert_status_record(icentris_client, 'running', table, file, get_datetime_now())
