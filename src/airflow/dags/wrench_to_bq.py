...
def get_running_files():
    ...
    query = BigQuery.querybuilder(
        select=['icentris_client', 'file', 'table'],
        where=[' '.join(where)],
        from_=f'{project_id}.wrench.data_source_status'
        )
...
def fetch_job_status(file_record):
    api_key = WrenchAuthWrapper.instance(env).get_api_key(file_record['icentris_client'])
    host = api_key['host']
    ...
    if response.status_code == requests.codes.ok:
        ...
        raise Exception(f'''Error pulling job-status from Wrench.
...
def fetch_entities(file_record):
    ...
    if response.status_code == requests.codes.processing or response.status_code == requests.codes.no_content:
        ...
        raise Exception(f'''Error pulling entities from Wrench.
                            Status: {response.status_code};
                            Body: {response.text}''')
...
def insert_entities(file_record, entities):
    for e in entities:
        e['icentris_client'] = file_record['icentris_client']
        e['ingestion_timestamp'] = str(datetime.utcnow())
        e['file'] = file_record['file']

    bq_client = BigQuery(env)
    query = QueryBuilder(
        table='wrench.entities',
        insert=entities
    )
...
def update_status(file_record, new_status):
    ...
    query = f"""UPDATE wrench.data_source_status SET
                end_date = '{status_date}'
                WHERE file = '{file_record["file"]}'
                AND status = 'running'
                AND end_date IS NULL"""
    bq_client.query(query)
    query = f"""INSERT INTO wrench.data_source_status(icentris_client, file, table, status, start_date) values(
            '{file_record['icentris_client']}', '{file_record["file"]}', '{file_record["table"]}',
...
def create_dag():
    ...
    with dag:
        ...
        for i in insight_ids:
            ...
            destination_table = f'wrench.{endpoint_name}'
