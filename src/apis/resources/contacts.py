...
class Contacts(Resource):
    ...
    def get(self):
        ...
        icentris_client = a.get_auth_icentris_client(domain)
        ...
        contacts = contacts_service.get_contacts(icentris_client=icentris_client, contact_ids=parse_csv(contact_ids))
        if contacts is None:
            return ResponseHelper.abort(410, msg=f'The recommendations for client {icentris_client} no longer exists')
        return contacts
    ...
    def post(self):
        ...
        try:
            for c in request_body['contacts']:
                ...
                Contact(**c)
                # Enrich payload
                c.update({'icentris_client': icentris_client, 'ingestion_timestamp': str(datetime.utcnow())})
