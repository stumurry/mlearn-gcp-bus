...
def up(client):
    ...
    if table.schema[-1].name == 'publish_date':
        logging.warning(f'publish_date already added to {table_name} in {dataset_name} dataset!')
        return dataset

    new_schema.append(bigquery.SchemaField('publish_date', 'DATETIME'))
