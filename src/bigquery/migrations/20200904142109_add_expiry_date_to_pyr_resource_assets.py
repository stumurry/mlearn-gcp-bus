...
def up(client):
    ...
    if table.schema[-1].name == 'expiry_date':
        logging.warning(f'expiry_date already added to {table_name} in {dataset_name} dataset!')
        return dataset

    new_schema.append(bigquery.SchemaField('expiry_date', 'DATETIME'))
