...
class ReadWrenchInsights(beam.DoFn):
    ...
    def process(self, element):
        ...
        api_key = WrenchAuthWrapper.instance(self._env).get_api_key()
        ...
        if response.status_code in ignored_codes:
            ...
            raise Exception(f'''Error reading insights from Wrench:
                                Url: {url};
                                Entities: {entity_ids};
    ...
    def process(self, element):
        ...
        if self._insight_id == 'match-score':
            ...
            for k, v in element.items():
                if k == 'entity_id':
                    continue
                scores.append({'icentris_client': k, 'score': int(v)})
...
class Runner():
    ...
    def _run(cls, p, options):
        with p:
            (
                ...
                | 'AddKeys' >> beam.Map(lambda x: (1, x))
                | 'Bundle' >> beam.GroupByKey()
    ...
    def run(cls):
        log = logging.getLogger()
        log.warning('>> Running Wrench to Bigquery Pipeline')
