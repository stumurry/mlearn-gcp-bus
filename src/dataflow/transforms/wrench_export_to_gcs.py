...
class UploadByKey(beam.DoFn):
    ...
    def csv_and_compress_elements(self, elements):
        content = ','.join(
            ['"' + str(k).replace('"', r'\"') + '"' for k in list(elements)[0].keys() if k not in ['icentris_client']])
        content += '\n'

        for e in elements:
            content += ','.join(
                ['"' + str(v).replace('"', r'\"') + '"' for k, v in e.items() if k not in ['icentris_client']])
            content += '\n'
    ...
    def process(self, element):
        ...
        icentris_client = element[0]
        content = self.csv_and_compress_elements(element[1])
        blob_name = f'{self._table}-{icentris_client}-{self._file}'
